...
class ImageElement(Widget):
    def __init__(
        ...
        width=100,
        height=100,
        fit=ft.ImageFit.CONTAIN,  # Ajuste de la imagen (CONTAIN, COVER, FILL, etc.)
        page=None,
    ):
    ...
    def create(self):
        return self.container  # Retornar el contenedor creado

    def update(self, src=None, width=None, height=None, fit=None):
        """Actualiza las propiedades de la imagen."""
        if src is not None:
            self.image.src = src  # Actualizar la ruta o URL de la imagen
        if width is not None:
            self.image.width = width  # Actualizar el ancho de la imagen
        if height is not None:
            self.image.height = height  # Actualizar el alto de la imagen
        if fit is not None:
            self.image.fit = fit  # Actualizar el ajuste de la imagen
